...
create table public.cameras (
  ...
  source_type text not null default 'webcam', -- webcam | upload | url
  source_url text,
...
-- Crowd readings (time-series)
create table public.crowd_readings (
  id bigserial primary key,
  camera_id uuid not null references public.cameras(id) on delete cascade,
  people_count int not null,
  density numeric not null,
  status text not null, -- safe | moderate | danger
...
create table public.alerts (
  id uuid primary key default gen_random_uuid(),
  camera_id uuid not null references public.cameras(id) on delete cascade,
  severity text not null, -- moderate | danger
  people_count int not null,
...
-- cameras: any authenticated can read; only admins write
...
-- crowd_readings: any auth read, any auth insert (operators sending data)
...
-- alerts: any auth read/insert; admin update (acknowledge)
...
begin
  ...
  -- First registered user becomes admin; everyone else 'authority'
